...
export default function Posts() {
    ...
    // List of prohibited words (from https://www.cs.cmu.edu/~biglou/resources/bad-words.txt)
    const rudeWords = `
abbo
abo
abortion
...
`.split("\n")
    ...
    // Open confirmation modal before delete or update
    ...
    // Set a post into editing mode
    const handleEditPost = (postId: number, currentContent: string) => {
        setEditingPostId(postId);
        setEditingContent(currentContent);
        // Reset update attempts for this editing session
        setUpdateAttempts(0);
    };
    ...
    // Save updated content by calling update API (after confirmation)
    const confirmUpdate = () => {
        if (confirmModalPostId === null) return;
        const foundRude = findRudeWords(editingContent);
        // If updated content contains rude words, increment update counter
        if (foundRude.length > 0) {
            setUpdateAttempts((prev) => prev + 1);
            if (updateAttempts + 1 >= 5) {
                // Delete the entire post after 5 inappropriate update attempts
                deletePost(confirmModalPostId, loggedInUser)
                    .then(() => {
        ...
        // Append " (edited)" if not already appended
    ...
    // When user clicks "Save" while editing, open the confirmation modal
    const handleUpdatePost = (postId: number) => {
        openConfirmModal("update", postId);
    };

    // When user clicks delete, open the confirmation modal
    ...
    return (
        <div className="container mx-auto grid grid-cols-12 gap-4 p-5">
                ...
                {posts.length > 0 ? (
                    posts.map((post: Post, index: number) => (
                      ...
                      {post.roseCount || 0}
                                ...
                                {/* For posts owned by the logged-in user, show update and delete icons */}
            ...
            {/* Right: Announcements Section (Green Theme) */}
